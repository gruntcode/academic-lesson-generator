...
DESCRIPTION_PATTERN = re.compile(r"Lesson Description:(.+?)(?=Learning Expectations:|$)", re.DOTALL)
EXPECTATIONS_PATTERN = re.compile(
    r"Learning Expectations:(.+?)(?=LESSON CONTENT:|LESSON OVERVIEW:|$)", re.DOTALL
)
# Strips list-bullet/numeric prefixes the LLM sometimes prepends to the
# description text (e.g. "- ", "* ", "1. ").
DESCRIPTION_PREFIX_PATTERN = re.compile(r"^\s*(?:[-*•]|\d+\.)\s*")
...
class PDFGenerator:
    ...
    def _create_styles(self):
        ...
        custom_styles = {
            ...
            "expectation_item": ParagraphStyle(
                "ExpectationItem",
                parent=base_styles["Normal"],
                fontSize=12,
                alignment=TA_LEFT,
    ...
    def _add_title_page(self, story, topic, grade_level, content):
        ...
        # Extract and add lesson description (presented as an italic blockquote-style
        # paragraph so it reads as an abstract rather than body text).
        description_match = DESCRIPTION_PATTERN.search(content)
        if description_match:
            description = self._clean_description(description_match.group(1))
            if description:
        ...
        # Extract and add learning expectations as a clean bullet list.
        ...
        if expectations_match:
            ...
            for line in expectations_match.group(1).split("\n"):
                line = line.strip()
                if not line:
                    continue
                # Strip leading bullets/numbers the LLM may have included.
                line = DESCRIPTION_PREFIX_PATTERN.sub("", line)
                if line:
                    story.append(Paragraph(line, self.styles["expectation_item"], bulletText="•"))

    def _clean_description(self, raw):
        """Normalise the lesson-description block into a single tidy paragraph."""
        lines = []
        for line in raw.splitlines():
            line = DESCRIPTION_PREFIX_PATTERN.sub("", line.strip())
            if line:
                lines.append(line)
        return " ".join(lines)
