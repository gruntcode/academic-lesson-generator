...
class TestGenerateLessonRoute:
    ...
    def test_generate_lesson_missing_topic(self, client):
        ...
        response = client.post(
            "/generate-lesson", data={"grade_level": "Middle School (Grades 6-8)"}
        )
    ...
    def test_generate_lesson_missing_grade_level(self, client):
        ...
        response = client.post("/generate-lesson", data={"topic": "Photosynthesis"})
        assert response.status_code == 400
        data = response.get_json()
        assert "error" in data
    ...
    def test_generate_lesson_topic_too_short(self, client):
        ...
        response = client.post(
            "/generate-lesson", data={"topic": "AB", "grade_level": "Middle School (Grades 6-8)"}
        )
        assert response.status_code == 400
        data = response.get_json()
    ...
    def test_generate_lesson_topic_too_long(self, client):
        ...
        response = client.post(
            "/generate-lesson",
            data={"topic": "A" * 201, "grade_level": "Middle School (Grades 6-8)"},
        )
        assert response.status_code == 400
        data = response.get_json()
    ...
    def test_generate_lesson_invalid_grade_level(self, client):
        ...
        response = client.post(
            "/generate-lesson", data={"topic": "Photosynthesis", "grade_level": "Invalid Grade"}
        )
        assert response.status_code == 400
        data = response.get_json()
    ...
    def test_generate_lesson_success(
        self, mock_pdf, mock_llm, client, sample_lesson_content, tmp_path
    ):
        ...
        response = client.post(
            "/generate-lesson",
            data={"topic": "Photosynthesis", "grade_level": "Middle School (Grades 6-8)"},
        )
    ...
    def test_generate_lesson_groq_api_error(self, mock_llm, client):
        ...
        mock_llm.generate_lesson_content.side_effect = groq.APIError(
            "API Error",
